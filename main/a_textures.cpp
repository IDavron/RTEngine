...
namespace {
    ...
    Group* imageTexturesCombine() {
        ...
        PerlinTexture* perlinTex = new PerlinTexture(RGBColor(0.0f,1.0f,0.0f), RGBColor(0.0f,0.0f,1.0f));
        perlinTex->addOctave(0.5f, 5.0f);
        perlinTex->addOctave(0.25f, 10.0f);
        perlinTex->addOctave(0.125f, 20.0f);
        perlinTex->addOctave(1.0f, 40.0f);
        FlatMaterial* perlin = new FlatMaterial(perlinTex);

        CombineMaterial* combine = new CombineMaterial();
        combine->add(near, 0.5f);
        combine->add(perlin, 0.5f);

        scene->add(new InfinitePlane(Point(0.1f, -1.5f,0.15f), Vector(0.0f, 0.0f, 1.0f), nullptr, combine));
...
void a_textures() {
    ...
    world.scene = imageTexturesCombine();
    engine.render(img);
    img.writeEXR("textures-combine.exr");
