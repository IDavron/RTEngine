...
void renderCornellbox(float scale, const char* filename, Camera* cam, Material* sphereMaterial, Material* floorMaterial, int numSamples=1) {
    ...
    // mirror
    // Material* mirror = new MirrorMaterial(0.5f,0);
    // scene->add(new Quad(Point(550.f, 250.f, 000.f)*scale, Vector(000.f, 000.f, 200.f)*scale, Vector(-200.f, 200.f, 000.f)*scale, nullptr, mirror)); //ceiling
...
void a_distributed() {
    ...
    Material* sphereMaterial3 = new ConductorMaterial(0.177, 3.638);
    ...
    //Mirror
    renderCornellbox(0.001f, "distributed-e.exr", cam, sphereMaterial3, floorMaterial1, 30);
}